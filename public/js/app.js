...
function cardVisualHTML(item, m, isYoutube) {
  ...
  const thumb = item.thumbnailUrl
    ...
    : '';
  // 유튜브는 플레이 버튼을 클릭하면 바로 원본 영상으로 이동
  const play = isYoutube && item.originalUrl
    ? `<a class="card-play" href="${esc(item.originalUrl)}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="원본 영상 보기"><i class="ti ti-player-play-filled"></i></a>`
    : isYoutube
      ? `<span class="card-play"><i class="ti ti-player-play-filled"></i></span>`
      : '';
  ...
  <article class="book-card ${esc(item.category)}${isUrl ? ' is-url' : ''}${featured ? ' featured' : ''}${hasInsight ? ' has-insight' : ''}" data-id="${esc(item.id)}" tabindex="0"
    ...
    <div class="card-body">
      ...
      <div class="book-inner">
        ...
        ${tagsHTML}
        ${isYoutube && item.originalUrl ? `<a class="card-yt-btn" href="${esc(item.originalUrl)}" target="_blank" rel="noopener" onclick="event.stopPropagation()"><i class="ti ti-brand-youtube"></i> 원본 영상 보기</a>` : ''}
        <div class="book-footer">
          <span class="book-date">${fmtDate(item.date)}</span>
          <div class="book-actions">
            <button class="book-btn" title="복사" onclick="event.stopPropagation();Library.copyItem('${esc(item.id)}',this)" aria-label="복사"><i class="ti ti-copy"></i></button>
...
const Library = {
  ...
  openDetail(id) {
    ...
    // URL 링크 (YouTube이면 빨간 버튼, 일반은 텍스트 링크)
    const isYtItem = item.category === 'youtube' || /youtube\.com|youtu\.be/i.test(item.originalUrl || '');
    const urlHtml = item.originalUrl
      ? isYtItem
        ? `<a class="detail-yt-btn" href="${esc(item.originalUrl)}" target="_blank" rel="noopener">
             <i class="ti ti-brand-youtube"></i> 📺 원본 영상 보기
           </a>`
    ...
    box.innerHTML = `
      ...
      <div class="detail-body">
        ${isYtItem && item.thumbnailUrl ? `<div class="detail-thumb-wrap"><img class="detail-thumb" src="${esc(item.thumbnailUrl)}" alt="" loading="lazy" onerror="this.parentElement.remove()"></div>` : ''}
  ...
  openGraph(id) {
    ...
    const isYtItem = center.type === 'youtube' || center.category === 'youtube';

    // 썸네일 + 유튜브 버튼
    const thumbWrap = el('kdYtThumbWrap');
    const thumbImg  = el('kdYtThumb');
    const playLink  = el('kdYtPlayOverlay');
    const ytBtn     = el('kdYtBtn');
    if (isYtItem && center.thumbnailUrl) {
      thumbImg.src = center.thumbnailUrl;
      if (center.originalUrl) { playLink.href = center.originalUrl; }
      thumbWrap.hidden = false;
    } else {
      thumbImg.src = '';
      thumbWrap.hidden = true;
    }
    if (isYtItem && center.originalUrl) {
      ytBtn.href   = center.originalUrl;
      ytBtn.hidden = false;
    } else {
      ytBtn.hidden = true;
    }

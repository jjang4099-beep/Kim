...
const { initConnection, getSQLiteDB, _persistDB, _sqlQuery, _sqlGet } = require('./db/connection');
...
/* ══════════════════════════════════════════════════
   인증 전역 가드 — /api/* 는 아래 화이트리스트를 제외하곤 모두 로그인 필요.
   화이트리스트: 인증 자체 라우트 + 순수 공용 참조 데이터(유저별 데이터 없음)
══════════════════════════════════════════════════ */
const PUBLIC_API_PATHS = new Set([
  '/api/auth/register', '/api/auth/login', '/api/auth/logout', '/api/auth/me',
  '/api/en-theme/today', '/api/en-theme/list',
  '/api/exam/daily-knowledge',
  '/api/lecture-recommend', '/api/log/lecture-click',
  '/api/push/vapid-key',
  '/health',
]);
app.use('/api', (req, res, next) => {
  /* app.use('/api', ...) 마운트 하위에서 req.path는 '/api' 프리픽스가 이미 잘려있음
     (예: '/api/auth/register' → '/auth/register') — 화이트리스트는 전체 경로 기준이므로
     originalUrl에서 쿼리스트링만 떼어내 비교한다 */
  const fullPath = req.originalUrl.split('?')[0];
  if (PUBLIC_API_PATHS.has(fullPath) || fullPath.startsWith('/api/country/')) return next();
...
async function initSQLiteDB() {
  await initConnection(SQLITE_PATH);
  getSQLiteDB().run(`
    /* ── 유저 아카이브 아이템 ──
    ...
    /* ══════════════════════════════════════════════════
       인증 — 이메일 로그인 유저 (구글/카카오는 provider 확장 시 추가)
    ══════════════════════════════════════════════════ */
    CREATE TABLE IF NOT EXISTS users (
      ...
      name          TEXT DEFAULT '',
      current_mode  TEXT NOT NULL DEFAULT 'PROFESSIONAL',
...
/** items.user_id 컬럼 추가(없으면) — 기존 행은 DEFAULT로 자동 백필됨 */
function _migrateItemsUserId() {
  try {
    const cols = getSQLiteDB().exec('PRAGMA table_info(items)');
    const hasUserId = cols.length && cols[0].values.some(row => row[1] === 'user_id');
    if (!hasUserId) {
      getSQLiteDB().run(`ALTER TABLE items ADD COLUMN user_id TEXT NOT NULL DEFAULT 'usr_legacy_owner'`);
      console.log('[SQLite] items.user_id 컬럼 추가 완료 (기본값: usr_legacy_owner)');
    }
    getSQLiteDB().run('CREATE INDEX IF NOT EXISTS idx_items_user_id ON items(user_id)');
    getSQLiteDB().run('CREATE INDEX IF NOT EXISTS idx_items_user_mode ON items(user_id, mode, created_at)');
  } catch (e) {
    console.warn('[SQLite] user_id 마이그레이션 실패 (무시):', e.message);
  }
}

/** user_categories.user_id 컬럼 추가(없으면) — 카테고리도 유저별로 격리 */
function _migrateCategoriesUserId() {
  try {
    const cols = getSQLiteDB().exec('PRAGMA table_info(user_categories)');
    const hasUserId = cols.length && cols[0].values.some(row => row[1] === 'user_id');
    if (!hasUserId) {
      getSQLiteDB().run(`ALTER TABLE user_categories ADD COLUMN user_id TEXT NOT NULL DEFAULT 'usr_legacy_owner'`);
      console.log('[SQLite] user_categories.user_id 컬럼 추가 완료');
    }
...
/**
 * 레거시 소유자 계정 자동 생성 — usr_legacy_owner에 실제 로그인 가능한 이메일/비번을 부여.
 * OWNER_EMAIL / OWNER_TEMP_PASSWORD 미설정 시 건너뜀(로컬 최초 부팅 등). 멱등.
 */
function _migrateCreateOwnerAccount() {
  try {
    const existing = _sqlGet('SELECT id FROM users WHERE id = ?', ['usr_legacy_owner']);
    if (existing) return;
    const email = process.env.OWNER_EMAIL;
    const pw    = process.env.OWNER_TEMP_PASSWORD;
    if (!email || !pw) {
      console.warn('[Auth] OWNER_EMAIL/OWNER_TEMP_PASSWORD 미설정 — 기존 데이터 소유 계정 생성 건너뜀');
      return;
    }
    const stmt = getSQLiteDB().prepare(
      'INSERT INTO users (id, email, password_hash, name, created_at, current_mode) VALUES (?,?,?,?,?,?)'
    );
    stmt.run(['usr_legacy_owner', email.toLowerCase().trim(), bcrypt.hashSync(pw, 10), 'SJ', new Date().toISOString(), 'PROFESSIONAL']);
    stmt.free();
    console.log(`[Auth] 기존 데이터 소유 계정 생성 완료 (${email})`);
  } catch (e) {
    console.warn('[Auth] 소유 계정 생성 실패 (무시):', e.message);
  }
}
...
function _migrateItemModes() {
  try {
    ...
    if (!hasMode) {
      getSQLiteDB().run(`ALTER TABLE items ADD COLUMN mode TEXT NOT NULL DEFAULT '${MODE_PRO}'`);
      console.log('[SQLite] items.mode 컬럼 추가 완료');
    }
    /* mode 인덱스 — 컬럼 보장 후 항상 생성 (신규/기존 DB 공통) */
    getSQLiteDB().run('CREATE INDEX IF NOT EXISTS idx_items_mode ON items(mode)');
    getSQLiteDB().run('CREATE INDEX IF NOT EXISTS idx_items_mode_created ON items(mode, created_at)');

    /* 2) 백필 — 데이터 JSON으로 모드 추론하여 컬럼 + JSON 동기화 */
    const rows = getSQLiteDB().exec('SELECT id, data, mode FROM items');
    if (!rows.length) return;
    const upd = getSQLiteDB().prepare('UPDATE items SET mode=?, data=? WHERE id=?');
    let migrated = 0;
...
// _persistDB / getSQLiteDB / _sqlQuery / _sqlGet → public/db/connection.js로 이관

function _migrateFromJSON() {
  const result = getSQLiteDB().exec('SELECT COUNT(*) AS c FROM items');
    ...
    try {
      ...
      for (const pack of (batch.exam_vocab_themes || [])) {
        ...
        for (const w of (pack.words || [])) {
          order++;
          const wStmt = getSQLiteDB().prepare(
...
/** 레거시 소유자용 — 서버 최초 부팅 시 테이블이 완전히 비어있을 때만 실행 */
function _seedDefaultCategories() {
  try {
    ...
    seedDefaultCategoriesForUser(DELIVERY_OWNER_ID);
...
/** 신규 회원가입 유저에게 기본 카테고리 세트를 복사 — 이미 있으면 스킵(멱등) */
function seedDefaultCategoriesForUser(userId) {
  const existing = _sqlGet('SELECT COUNT(*) AS c FROM user_categories WHERE user_id = ?', [userId]);
  if (existing && Number(existing.c) > 0) return;
  const stmt = getSQLiteDB().prepare(
    'INSERT INTO user_categories (name, emoji, color, sort_order, is_default, modes, user_id) VALUES (?,?,?,?,?,?,?)'
  );
  for (const d of DEFAULT_CATEGORIES) stmt.run([d.name, d.emoji, d.color, d.order, d.is_default, d.modes, userId]);
  stmt.free();
  _persistDB();
}
...
function _migrateLegacyDomains() {
  const result = getSQLiteDB().exec("SELECT id, data FROM items WHERE category IN ('en','zh','history','economy','youtube','inbox') OR category IS NULL");
...
// Summaries DB(결산 캐시) — public/db/summaries.js로 이관 (userId 포함 캐시 키)
...
/** 공용 피드 캐시(feed)에서 유저별 저장 상태(saved/savedItemId)만 파생시켜 응답용 사본 반환 */
function withSaveState(feed, userId) {
  if (!feed) return feed;
  const savedId = feed.savedBy ? feed.savedBy[userId] : null;
  return { ...feed, saved: !!savedId, savedItemId: savedId || null };
}
 ...
 */
/* 이 파이프라인은 단일 delivery-settings 오너(getDefaultUser)에 묶인 레거시 기능이라
   usr_legacy_owner 소유 아이템만 대상으로 한다 — 그러지 않으면 여러 신규 유저의
   비공개 아카이브 내용이 한 AI 프롬프트에 섞여 들어가는 교차 유저 유출이 된다. */
const DELIVERY_OWNER_ID = 'usr_legacy_owner';
...
async function generateDailyDelivery(user, force = false) {
  ...
  if (!force) {
    const existing = ItemsDB.getItemsByUser(DELIVERY_OWNER_ID)
      .filter(i => i.type === 'daily_delivery' && i.date === today);
  ...
  const recentItems = ItemsDB.getItemsByUser(DELIVERY_OWNER_ID, MODE_PRO)
    .filter(i => i.date >= weekAgoStr && i.type !== 'daily_delivery')
    .slice(0, 20);
...
[{"title":"카드 제목(20자 이내)","category":"en|economy|history|youtube|inbox","summary3":"• 요약1\\n• 요약2\\n• 요약3","concepts":[{"term":"개념 또는 영어 표현","desc":"설명 1~2문장"},{"term":"개념2","desc":"설명"}],"reminder":"오늘 하루 실전 적용 한 줄"}]`;
  ...
  // force 모드: 기존 오늘 배달 카드 먼저 삭제 (오너 소유분만)
  if (force) {
    for (const it of ItemsDB.getItemsByUser(DELIVERY_OWNER_ID).filter(i => i.type === 'daily_delivery' && i.date === today)) {
      ItemsDB.deleteItemForUser(DELIVERY_OWNER_ID, it.id);
    }
  }
  // 각 카드를 개별 삽입 (오너 소유로 적재)
  for (const card of newCards) ItemsDB.insertItem(DELIVERY_OWNER_ID, card);
...
// ══════════════════════════════════════════════════
//  이메일 인증 (회원가입/로그인/로그아웃/me/모드전환)
//  구글/카카오 소셜 로그인은 앱스토어 출시 시점에 별도 구현 (지금은 범위 밖)
// ══════════════════════════════════════════════════

function toPublicUser(user) {
  return { id: user.id, email: user.email, name: user.name, currentMode: user.current_mode };
}

app.post('/api/auth/register', (req, res) => {
  const { email, password, name } = req.body || {};
  if (!email || !password) return res.status(400).json({ success: false, error: '이메일과 비밀번호를 입력해주세요' });
  if (password.length < 8) return res.status(400).json({ success: false, error: '비밀번호는 8자 이상이어야 해요' });
  if (UsersDB.findUserByEmail(email)) return res.status(409).json({ success: false, error: '이미 가입된 이메일이에요' });
  const user = UsersDB.createUser({ email, password, name });
...
app.post('/api/auth/login', (req, res) => {
  ...
  const user = email && UsersDB.findUserByEmail(email);
  if (!user || !bcrypt.compareSync(password || '', user.password_hash)) {
    return res.status(401).json({ success: false, error: '이메일 또는 비밀번호가 올바르지 않아요' });
  }
...
app.get('/api/auth/me', requireAuth, (req, res) => {
  const user = UsersDB.findUserById(req.userId);
  if (!user) return res.status(401).json({ success: false, error: '유저를 찾을 수 없어요' });
  res.json({ success: true, user: toPublicUser(user) });
});
...
app.delete('/api/push/subscribe', (req, res) => {
  ...
  const subs    = readPushSubs();
  const cleaned = subs.filter(s => !(s.subscription.endpoint === endpoint && s.userId === req.userId));
  writePushSubs(cleaned);
...
app.post('/api/inbox', async (req, res) => {
  ...
  const userId = req.userId;
  ItemsDB.insertItem(userId, newItem);
  ...
  setImmediate(async () => {
    try {
      ...
      if (c.category !== 'inbox') {
        ...
        ItemsDB.updateItemForUser(userId, newItem);
...
app.get('/api/daily-feed', async (req, res) => {
  ...
  if (!force) {
    ...
    if (cached && Object.keys(cached).length > 0) {
      ...
      if (!missingSubs.length) {
        ...
        const feeds = Object.values(cached).map(f => withSaveState(f, req.userId));
      ...
      // 전체 캐시(기존 + 새로 생성) 반환
      const allCached = Object.values(getTodayFeeds(today) || {}).map(f => withSaveState(f, req.userId));
      return res.json({
  ...
  try {
    ...
    const arr   = Object.values(feeds).map(f => withSaveState(f, req.userId));
    res.json({
      success:   true,
      date:      today,
      cached:    false,
...
app.post('/api/daily-feed/generate', async (req, res) => {
  ...
  try {
    ...
    const feedsArr = Object.values(allFeeds).map(f => withSaveState(f, req.userId));
    res.json({
      success: true,
      date:    toDateStr(),
...
app.get('/api/user/status', (req, res) => {
  ...
  const count = ItemsDB.getItemsByUser(req.userId, mode).length;
...
app.post('/api/daily-feed/:date/:subId/save', async (req, res) => {
  ...
  /* dailyFeeds.json은 전체 유저 공용 캐시(콘텐츠 자체는 공유) — "저장했는지" 여부만 유저별로 격리 */
  const existingSavedId = feed.savedBy?.[userId];
  if (existingSavedId) return res.json({ success: true, alreadySaved: true, itemId: existingSavedId });
  ...
  // dailyFeeds에 유저별 저장 상태 기록 (콘텐츠 자체는 전 유저 공용, 저장 여부만 격리)
  if (!all[date][subId].savedBy) all[date][subId].savedBy = {};
  all[date][subId].savedBy[userId] = newItem.id;
  writeDailyFeeds(all);
...
app.get('/api/items', (req, res) => {
  ...
  /* ── 모드 격리(Isolation) + 유저 격리 ── : mode 파라미터가 오면 DB 단계에서 원천 분리.
     수험생 모드 조회 시 직장인 데이터는 쿼리 자체에서 배제(WHERE mode=?, 인덱스 활용). */
  let items = ItemsDB.getItemsByUser(req.userId, mode);
...
app.get('/api/items/life', (req, res) => {
  try {
    ...
    let items = ItemsDB.getItemsByUser(req.userId).filter(i => i.contentType === 'life');
...
app.post('/api/items/life', upload.array('photos', 10), (req, res) => {
  try {
    ...
    ItemsDB.insertItem(req.userId, item);
...
app.get('/api/items/:id', (req, res) => {
  const item = ItemsDB.getItemByIdForUser(req.userId, req.params.id);
...
app.post('/api/quiz/generate', async (req, res) => {
  ...
  let items = ItemsDB.getItemsByUser(req.userId).filter(i => i.type !== 'daily_delivery');
...
app.delete('/api/items/:id', (req, res) => {
  const deleted = ItemsDB.deleteItemForUser(req.userId, req.params.id);
  if (!deleted) return res.status(404).json({ success: false, error: '항목을 찾을 수 없습니다.' });
  res.json({ success: true });
});
...
app.post('/api/summary', async (req, res) => {
  ...
  // ── 데이터 필터링 ──
  let items = ItemsDB.getItemsByUser(req.userId);
...
async function analyzeStoredExamItemById(userId, itemId) {
  const item = ItemsDB.getItemByIdForUser(userId, itemId);
  ...
  item.analysisStatus = 'analyzing'; item.updatedAt = new Date().toISOString(); ItemsDB.updateItemForUser(userId, item);
  try {
    const a = await runExamImageAnalysis(stored, subject, item.userHint || '');
    const fresh = ItemsDB.getItemByIdForUser(userId, itemId) || item;
    if (a) { applyExamAnalysis(fresh, a, subject); }
    else   { fresh.analysisStatus = 'failed'; fresh.updatedAt = new Date().toISOString(); }
    ItemsDB.updateItemForUser(userId, fresh);
    console.log(`[오답분석] ${itemId} → ${fresh.analysisStatus}`);
  } catch (e) {
    console.error('[오답분석] 실패:', e.message);
    const fresh = ItemsDB.getItemByIdForUser(userId, itemId);
    if (fresh) { fresh.analysisStatus = 'failed'; fresh.updatedAt = new Date().toISOString(); ItemsDB.updateItemForUser(userId, fresh); }
  }
}
...
app.post('/api/analyze-image', upload.array('image', 10), async (req, res) => {
  ...
  try {
    ...
    if (action === 'store') {
      items.forEach(it => { it.analysisStatus = 'pending'; ItemsDB.insertItem(req.userId, it); });
      ...
      if (process.env.GEMINI_API_KEY) {
        ...
        const userId = req.userId;
        setImmediate(async () => {
          for (const it of items) { await analyzeStoredExamItemById(userId, it.id).catch(() => {}); }
        });
      }
      return;
    }
    ...
    await Promise.all(items.map(async (it, i) => {
      ...
      ItemsDB.insertItem(req.userId, it);
    }));
...
app.get('/api/exam/wrong/export-pdf', async (req, res) => {
  try {
    ...
    let items = ItemsDB.getItemsByUser(req.userId, MODE_EXAM).filter(i => i.type === 'wrong_answer');
    if (subject !== 'all') {
      items = subject === 'etc'
...
app.post('/api/exam/wrong/:id/analyze', async (req, res) => {
  ...
  await analyzeStoredExamItemById(req.userId, item.id);
  const updated = ItemsDB.getItemByIdForUser(req.userId, item.id);
...
app.post('/api/exam/daily-knowledge/save', (req, res) => {
  ...
  try {
    ...
    /* 중복 저장 방지 — 동일 id로 이 유저가 이미 저장했으면 그대로 성공 반환
       (id는 exw_/exh_ 접두 결정적 id라 유저별 체크 없인 다른 유저 저장분과 충돌한다) */
    const exists = _sqlGet('SELECT id FROM items WHERE id = ? AND user_id = ?', [item.id, req.userId]);
...
/* GET /api/exam/settings — 유저별 D-day 설정 */
app.get('/api/exam/settings', (req, res) => {
  const all = readJSON(EXAM_SETTINGS_PATH, {});
  const settings = all[req.userId] || { examDate: null, examName: '' };
...
app.post('/api/exam/settings', (req, res) => {
  ...
  all[req.userId] = settings;
  writeJSON(EXAM_SETTINGS_PATH, all);
  res.json({ success: true, ...settings });
});

/* GET /api/exam/today-summary */
app.get('/api/exam/today-summary', (req, res) => {
  const items = ItemsDB.getItemsByUser(req.userId, MODE_EXAM);
  const now   = new Date();
...
app.get('/api/exam/weakness-analysis', (req, res) => {
  ...
  const items = ItemsDB.getItemsByUser(req.userId, MODE_EXAM).filter(i =>
...
app.get('/api/categories', (req, res) => {
  try {
    res.json(CategoriesDB.getCategoriesForUser(req.userId));
  } catch (e) {
...
app.post('/api/categories', (req, res) => {
  ...
  try {
    const existing = CategoriesDB.getCategoriesForUser(req.userId);
    const nextOrd  = existing.reduce((m, c) => Math.max(m, Number(c.sort_order) || 0), 0) + 1;
    const newCat   = CategoriesDB.insertCategory(req.userId, { name, emoji, color, sortOrder: nextOrd, modes: 'both' });
    res.json(newCat);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.patch('/api/categories/:id', (req, res) => {
  ...
  try {
    ...
    if (!Object.keys(fields).length) return res.status(400).json({ error: '수정할 필드가 없습니다' });
    const ok = CategoriesDB.updateCategoryForUser(req.userId, req.params.id, fields);
...
app.delete('/api/categories/:id', (req, res) => {
  try {
    /* 포함 아이템의 userCategoryId를 null로 초기화 (본인 소유 아이템만) */
    const affected = ItemsDB.getItemsByUser(req.userId).filter(i => String(i.userCategoryId) === req.params.id);
    ...
    const ok = CategoriesDB.deleteCategoryForUser(req.userId, req.params.id);
    if (!ok) return res.status(404).json({ error: '카테고리를 찾을 수 없습니다' });
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.get('/api/summary/:type/:period', async (req, res) => {
  ...
  /* 모드 + 유저 격리 — 캐시 키에 userId까지 포함해야 서로 다른 유저 결산이 안 섞임 */
  const mode = normalizeMode(req.query.mode);

  /* 캐시 반환 */
  const cached = SummariesDB.getCachedSummary(req.userId, mode, type, period);
  ...
  try {
    ...
    const allItems  = ItemsDB.getItemsByUser(req.userId, mode).filter(i => {
    ...
    SummariesDB.setCachedSummary(req.userId, mode, type, period, result);
    res.json(result);
  } catch (e) {
